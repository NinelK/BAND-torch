...
def R2(pred_beh, true_beh):
    return (
        1
        - np.sum((pred_beh - true_beh) ** 2)
        / np.sum((true_beh - np.mean(true_beh)) ** 2)
    ) * 100
...
def get_target_ids(true_target_direction):
    ...
    true_label = np.array(
        [np.where(uniq_dirs == t)[0][0] for t in true_target_direction]
    )
    return true_label
...
def plot_avg_traj(data, true_target_direction, title="", epoch_mask=True, sharey=False):
    ...
    fig, ax = plt.subplots(
        row, col, figsize=(col * 3, row * 2), sharex=True, sharey=sharey
    )
    if type(ax) is Axes:
        ax = [ax]
    else:
    ...
    for i in range(n):
        for j in np.unique(true_label):
            ax[i].plot(
                data[(true_label == j) & epoch_mask].mean(0)[..., i],
                label=f"{true_target_direction[true_label==j][0]:.2f}",
            )
...
def plot_beh_pred(vel, pred_vel, dir_index, trials2plot, file_name=""):
    ...
    axes[0].text(
        np.min(pos[..., 0]), np.max(pos[..., 1]), f"R2_pos = {R2_iso_pos*100:.2f}%"
    )
...
if "lfads" in model_name:
    overrides["model.behavior_weight"] = 0.0
...
# load the dataset
if fold is not None:
    datafile_pattern = config.datamodule.datafile_pattern.replace(
        ".h5", f"_cv{fold}.h5"
    )
else:
...
for sess_id, dataset_filename in enumerate(data_paths):
    ...
    if "pbt" in PROJECT_STR:
        data_path = (
            ...
            + "/"
            + model_name
            + f"/best_model/lfads_output_{session}.h5"
        )
    else:
    ...
    if co_dim > 0:
        ...
        if "pbt" in PROJECT_STR:
            data_path = (
                ...
                + "/"
                + model_name
                + f"/best_model/lfads_W_ablated_output_{session}.h5"
            )
        else:
            data_path = (
                best_model_dest
                + "/"
                + model_name
                + f"/lfads_W_ablated_output_{session}.h5"
            )
        with h5py.File(data_path) as f:
    ...
    valid_LL = {
        "LL": poisson.logpmf(valid_data, bin_width_sec * rates),
        "LL_noci": poisson.logpmf(valid_data, bin_width_sec * noci_rates),
    }
    valid_co_bps = {
        key: np.sum(valid_LL[key] - base_LL) / (n_sp * np.log(2))
        for key in valid_LL.keys()
    }
    # add same AD
    valid_co_bps_AD = {
        key
        + "_AD": np.sum(valid_LL[key][valid_epoch == 1] - base_LL[valid_epoch == 1])
        / (n_sp * np.log(2))
        for key in valid_LL.keys()
    }
    ...
    if co_dim > 0:
        Y_pred_noci_0lag = ridge.predict(
            noci_factors.reshape(-1, noci_factors.shape[-1])
        ).reshape(true_valid_beh.shape)
    ...
    if co_dim > 0:
        Y_pred_noci_seq2seq = ridge.predict(
            noci_factors.reshape(noci_factors.shape[0], -1)
        ).reshape(true_valid_beh.shape)
    ...
    def save_results(
        f, area, mn, fac_dim, co_dim, train_outputs, test_outputs, sample=""
    ):
        ...
        if key in f:
            del f[key]
        f.create_dataset(
            f"train_{area}_{mn}_{fac_dim}f_{co_dim}c{sample_str}_pred",
            data=train_outputs,
        )
        f.create_dataset(
            f"test_{area}_{mn}_{fac_dim}f_{co_dim}c{sample_str}_pred", data=test_outputs
        )

    short_dataset_name = (
        dataset_name.replace("_M1", "").replace("_PMd", "").replace("_small", "")
    )
    ...
    if type(model.behavior_readout) == FanInSeq2Seq:
        ...
        r = torch.std(beh_W) * 4
        fig, ax = plt.subplots(
            out_features, in_features, figsize=(in_features, out_features)
        )
        ...
        plt.imshow(
            beh_W.detach().numpy().T, cmap="RdBu", vmin=-r, vmax=r, interpolation="none"
        )
    ...
    ax[0, 1].set_title(
        f"0lag from factors (R2 = {R2(Y_pred_0lag,true_valid_beh):.1f}%)"
    )
    ax[0, 2].set_title(
        f"seq2seq from factors (R2 = {R2(Y_pred_seq2seq,true_valid_beh):.1f}%)"
    )
    ax[0, 3].set_title(f"band behavior (R2 = {R2(behavior,true_valid_beh):.1f}%)")
    if co_dim > 0:
        ax[1, 1].set_title(
            f"0lag from controls (R2 = {R2(Y_pred_control_0lag,true_valid_beh):.1f}%)"
        )
        ax[1, 2].set_title(
            f"seq2seq from controls (R2 = {R2(Y_pred_control,true_valid_beh):.1f}%)"
        )
        ax[2, 1].set_title(
            f"0lag from factors no CI (R2 = {R2(Y_pred_noci_0lag,true_valid_beh):.1f}%)"
        )
        ax[2, 2].set_title(
            f"seq2seq from factors no CI (R2 = {R2(Y_pred_noci_seq2seq,true_valid_beh):.1f}%)"
        )
        ax[2, 3].set_title(
            f"band behavior no CI (R2 = {R2(noci_behavior,true_valid_beh):.1f}%)"
        )

    # combine R2 results and save as csv
    R2_results = {
        "seq2seq from factors": R2(Y_pred_seq2seq, true_valid_beh),
        "seq2seq from factors no CI": R2(Y_pred_noci_seq2seq, true_valid_beh),
        "seq2seq from factprs in AD": R2(
            Y_pred_seq2seq[valid_epoch == 1], true_valid_beh[valid_epoch == 1]
        ),
        "seq2seq from factors no CI in AD": R2(
            Y_pred_noci_seq2seq[valid_epoch == 1], true_valid_beh[valid_epoch == 1]
        ),
        "band behavior": R2(behavior, true_valid_beh),
        "band behavior no CI": R2(noci_behavior, true_valid_beh),
        "band behavior in AD": R2(
            behavior[valid_epoch == 1], true_valid_beh[valid_epoch == 1]
        ),
        "band behavior no CI in AD": R2(
            noci_behavior[valid_epoch == 1], true_valid_beh[valid_epoch == 1]
        ),
