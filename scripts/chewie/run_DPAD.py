...
def save_results(f, area, train_outputs, test_outputs):
    if f"train_{area}_dpad_pred" in f:
        del f[f"train_{area}_dpad_pred"]
    if f"test_{area}_dpad_pred" in f:
        del f[f"test_{area}_dpad_pred"]
    f.create_dataset(f"train_{area}_dpad_pred", data=train_outputs)
    f.create_dataset(f"test_{area}_dpad_pred", data=test_outputs)
...
# save summary
with open("./results/DPAD_decoder_R2.csv", "w") as f:
