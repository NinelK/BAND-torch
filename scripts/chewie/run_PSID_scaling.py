...
def save_results(f,area,train_outputs, test_outputs, n_factors):
    if f'train_{area}_psid_{n_factors}f_pred' in f:
        del f[f'train_{area}_psid_{n_factors}f_pred']
    if f'test_{area}_psid_{n_factors}f_pred' in f:
        del f[f'test_{area}_psid_{n_factors}f_pred']
    f.create_dataset(f'train_{area}_psid_{n_factors}f_pred', data=train_outputs)
    f.create_dataset(f'test_{area}_psid_{n_factors}f_pred', data=test_outputs)

summary_dict = {}   
short_dataset_name = experiments[3]

for n_factors in tqdm([100]):

    summary_dict[n_factors] = {}
    ...
    for area in ['M1','all']:
        ...
        summary_dict[n_factors][f"{area}_R2_all"] = np.round(100*final_r2,1) 

        for e, epoch_name in enumerate(['BL','AD','WO']):
            final_r2 = r2_score(valid_behavior[valid_epoch==e], test_outputs[valid_epoch==e]).item()
            summary_dict[n_factors][f"{area}_R2_{epoch_name}"] = np.round(100*final_r2,1)       

        # save predictions
        results_path = f'./results/{short_dataset_name}.h5'
        with h5py.File(results_path, 'a') as f:
            save_results(f,area,train_outputs, test_outputs,n_factors)

# save summary
with open("./results/PSID_decoder_R2_scaling.csv", "w") as f:
    get_column_names = summary_dict[4].keys()
    f.write("Dataset,")
