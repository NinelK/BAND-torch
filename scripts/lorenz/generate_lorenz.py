...
def generate_lorenz_dataset(
    base_dir, bin_sz_ms=10, delay_bins=0, n_behavior=2, behavior_noise_std=0.1
):
    """
    ...
    {dataset_name}_test.h5  : Train + adaptation-test splits for evaluation.
     ...
     _epoch: array saved in the HDF5 datasets. Boolean (1 for perturbed, 0 for unperturbed) 
     indicating whether a perturbation input/kick occurred for that trial.
    """

    # delay_bins = -10                # number of time bins to delay the input

    if delay_bins > 0:
        dataset_name = f"data_Synthetic_Lorenz_behrel_off0_forward_{np.abs(delay_bins)}"
    elif delay_bins == 0:
        dataset_name = f"data_Synthetic_Lorenz_behrel_off0_0"
    else:
        dataset_name = (
            f"data_Synthetic_Lorenz_behrel_off0_feedback_{np.abs(delay_bins)}"
        )
    ...
    C_vel_pinv = torch.linalg.pinv(C_vel)
    direction_vec = (
        C_vel_pinv @ C_vel @ torch.randn(n_latents_true)
    ).numpy()  # behaviorally-relevant perturbation
    ...
    def simulate_lorenz(n_trials, perturbed_ratio=0.0):
        ...
        kick_prob = 0.05
        kick_magnitude = 5.0
        off_manifold_kick_magnitude = 0.0
        ...
        for _ in range(n_trials):
            ...
            for b in range(n_bins_sim):
                ...
                # NOTE: averaging over fine steps: Within each time bin, the Lorenz ODE 
                # is integrated at a much finer resolution
                # For dt_integration = 0.005s, there are 4 fine steps per 20ms bin
                # We average those integration steps to get a single representative 
                # latent state for the bin.
                z_bin_raw = np.mean(bin_states[:-1], axis=0)
                z_bin_scaled = z_bin_raw / 10.0
                z_bin_shifted = z_bin_scaled + np.array([2, 2, -0.5])
                ...
                vel = torch.matmul(
                    z_tensor, C_vel.T
                ) + behavior_noise_std * torch.randn(n_behavior)
            ...
            v_sliced = v_stacked[
                max_delay_bins - delay_bins : max_delay_bins - delay_bins + n_bins
            ]
    ...
    def save_h5(
        ...
        with h5py.File(filename, "w") as h5file:
            ...
            # Validation/Test data may vary based on splits with different pertubation
            ...
            h5file.create_dataset(
                "params/ortho_input_vec", data=ortho_input_vec.numpy()
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--delay_bins", type=int, default=0, help="Number of bins to delay behavior"
    )
    parser.add_argument(
        "--n_behavior",
        type=int,
        default=2,
        help="Behavioral output dimensionality (< n_latents_true)",
    )
    parser.add_argument(
        "--behavior_noise_std",
        type=float,
        default=0.1,
        help="Std of Gaussian noise on behavior",
    )
