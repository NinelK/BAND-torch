...
from .band2_tuples import SessionBatch, SessionOutput
from .band_utils import transpose_lists
from .metrics import ExpSmoothedMetric, r2_score, regional_bits_per_spike
from .modules import band_augmentations
...
class BAND2(pl.LightningModule):
    def __init__(
        self,
        encod_data_dim: int,
        encod_seq_len: int,
        ...
        cell_clip: float,
        train_aug_stack: band_augmentations.AugmentationStack,
        infer_aug_stack: band_augmentations.AugmentationStack,
    ...
    def forward(
        ...
        noci_output_params = [
            self.recon[s].reshape_output_params(op)
            for s, op in zip(sessions, noci_output_params)
        ]
    ...
    def _shared_step(self, batch, batch_idx, split):
        ...
        noci_recon_all = [
            self.recon[s].compute_loss(batch[s].recon_data, output[s].noci_output_params)
            for s in sessions
        ]
        ...
        noci_behavior_recon_all = [
            self.beh_recon[s].compute_loss(
                batch[s].behavior,
                output[s].noci_output_behavior_params[:, : batch[s].behavior.shape[1]][
                    ..., None
                ],
            )
            for s in sessions
        ...
        if not hps.recon_reduce_mean:
            ...
            noci_recon_all = [torch.sum(ra, dim=(1, 2)) for ra in noci_recon_all]
        ...
        sess_noci_recon = [ra.mean() for ra in noci_recon_all]
        sess_noci_behavior_recon = [ra.mean() for ra in noci_behavior_recon_all]
        recon = torch.mean(torch.stack(sess_recon))
        behavior_recon = torch.mean(torch.stack(sess_behavior_recon))
        noci_recon = torch.mean(torch.stack(sess_noci_recon))
        noci_behavior_recon = torch.mean(torch.stack(sess_noci_behavior_recon))
        ...
        loss = hps.loss_scale * (
            ...
            + self.hparams.behavior_weight * noci_behavior_recon
            + l2_ramp * l2
        ...
        metrics = {
            ...
            f"{split}/noci_recon": noci_recon,
            f"{split}/noci_beh_recon": noci_behavior_recon,
